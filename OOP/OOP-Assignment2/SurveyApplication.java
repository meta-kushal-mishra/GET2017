...
public class SurveyApplication {
	...
	private ReadQuestions readQuestions; // Object to read the questions from
											// the file
	private CreateAnswerReport createReport; // Object to create the report of
												// the answers
	private String[] lines; // Array to store the questions
	private Question[] question; // Array to store the various parts of a
									// question
	...
	private int numberOfParticipants; // To store the total number of
										// participants
	private List<SingleSelectReport> report; // List to store the objects of
												// single select questions
	private SingleSelectReport reportObject; // Object for generating the single
												// select question
	...
	public Question[] separateLines(String[] lines) {
		...
		for (int i = 0; i < lines.length; i++) {
			...
			if (temp.length == 3) {
				question[i].setChoices(temp[2]); // Third part refers to the
													// choices available
			}
		}
		return question;
	...
	// method to get answers from user for each question and returns string of
	// that answer takes participant no. as argument
	private String getAnswers(int participantNo) {
		String ans = "Participant " + (participantNo + 1); // The final answer
															// for the
															// participant
		String[] temp;
		...
		for (int i = 0; i < question.length; i++) { // loop to print all the
													// questions
			System.out.println(question[i].getQuestion()); // Printing the
															// question

			while (true) { // loop to ensure that the input entered is correct

				// For printing the options for multiple and single select type
				// question
				if ((question[i].getType().equalsIgnoreCase("Multi Select"))
					...
					if (ansSet == null) {
						// Answer is not correct and continue to take input for
						// user for the same question
						...
						if (question[i].getType().equalsIgnoreCase(
								"Single Select")) {
							report.get(singleSelectQuesNo).storeAnswer(ansSet);
							singleSelectQuesNo++;
						}

						ans = ans + "," + ansSet; // appending the sub answer to
													// main answer string
					...
					if (question[i].getType().equalsIgnoreCase("Text")) {
						...
						ans = ans + "," + scan.nextLine(); // appending the sub
															// answer to main
															// answer string
						break; // Answer is correct move to next question
					} else {
						...
						// action to perform based on validation of answer for
						// numeric type
						if (ansSet == null) {
							System.out.println("Enter numerals only");
							continue;
						} else {
							ans = ans + "," + ansSet; // appending the sub
														// answer to main answer
														// string
							break; // Answer is correct move to next question
						}
					}
	...
	public void fillSurveyForm() throws IOException {
		...
		// call to separate the questions, type and options from the lines in
		// input files
		separateLines(lines);

		System.out.print("Enter number of participants in the survey ");
		numberOfParticipants = scan.nextInt(); // Get the number of participant
												// from user
		scan.nextLine(); // to consume extra \n in the buffer
		answers = new String[numberOfParticipants]; // initialize the toatl
													// number of answers
		...
		for (int i = 0; i < question.length; i++) {
			if (question[i].getType().equalsIgnoreCase("Single Select")) {
				...
				reportObject = new SingleSelectReport(
						question[i].getQuestion(), question[i].getType(),
						question[i].getChoices());
				report.add(reportObject); // Adding the single select object to
											// list
			}
		}
		...
		for (int i = 0; i < report.size(); i++) {
			System.out.println(report.get(i).generateReport(
